...
public class AlgoritmoDeBlockchainImpl implements AlgoritmoDeBlockchain {
    ...
    private void construirBlockchainRecursivo(List<Transaccion> transacciones, int maxTamanioBloque,// Backtracking
        ...
        if (esBloqueValido(transaccionesActuales, maxTamanioBloque, maxValorBloque, maxTransacciones)) {
            ...
            Bloque nBloque = new Bloque();

            nBloque.setValorTotal(bloqueActual.getValorTotal());
            nBloque.setTamanioTotal(bloqueActual.getValorTotal());
            nBloque.setTransacciones(bloqueActual.getTransacciones());

            bloquesActuales.add(nBloque); // Agrego el nuevo bloque solo si no existe en la lista

            List<Bloque> nBloquesActuales = new ArrayList<Bloque>(bloquesActuales);

            soluciones.add(nBloquesActuales); // Se agrega como posible solucion
            ...
            for (int i = indice; i < transacciones.size(); i++) {
                ...
                if (sePuedeAgregar(transaccion, transaccionesActuales)) { // Verifica si cumple con los permisos para guardarolo
                    ...
                    List<Transaccion> ntransaccionesActuales = new ArrayList<Transaccion>(transaccionesActuales);

                    bloqueActual.setTransacciones(ntransaccionesActuales); // Agrega las transacciones al bloque
                    ...
                    ntransaccionesActuales = new ArrayList<Transaccion>(transaccionesActuales);
                    bloqueActual.setTransacciones(ntransaccionesActuales); // Saca la ultima transaccion del bloque
